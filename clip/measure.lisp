...
(define-experiment vary-threads (duration clients)
...
(let ((clasp-file (merge-pathnames "data.clasp" *load-pathname*)))
  (when (probe-file clasp-file)
    (delete-file clasp-file))
  (handler-bind ((error (lambda (e) (print e) (clip:shutdown-and-run-next-trial))))
    (clip:run-experiment 'vary-threads :args '(1 1) :output-file clasp-file)
    (clip:run-experiment 'vary-threads :args '(1 5) :output-file clasp-file)))
